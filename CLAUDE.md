...
-   [CCWEB2-328](https://codecave.atlassian.net/browse/CCWEB2-328) — **fixed, awaiting a PR.** 32 of the site's 33 pages shipped with no `<title>`, no description and no canonical, and the one canonical that existed said `http://localhost:4321/` — `SeoPage` built it from `Astro.url.origin`, which during a static build is the dev server. `SeoPage` now takes the full head (canonical, robots, Open Graph, Twitter card, article timestamps) off `Astro.site`, every page passes it, and `scripts/check-seo.mjs` fails the build if one does not. Setting `site` also un-broke `@astrojs/sitemap`, which had been silently skipping every build. Branch `ccweb2-328-seo-meta`, commit `58bd677e`. Two things still need a human: the default social image is the square PWA icon standing in for a real 1200×630 card, and Strapi's tag casing is inconsistent (`ai` next to `3D`), which shows in the tag pages' titles.
