...
-   [CCWEB2-374](https://codecave.atlassian.net/browse/CCWEB2-374) — **still an open decision, but the reason given for closing it was wrong, and there is now a branch that proves it.** The docs site reads like a documentation site and is not one — all 30 pages are specimens or galleries, and the only real prose it has (`DESIGN.md`, `README.md`, `SKILL.md`, `guide.md`, 1,601 lines) is not rendered as pages at all.
    ...
    `spike/starlight-ccweb2-374` is no longer a spike — it is a working adoption, and `STARLIGHT-SPIKE.md` carries every measurement. **It is still unmerged, and merging it is still the decision**; what changed is what the decision is about. Not "would this work" — it does — but whether a third surface and a Starlight dependency are wanted.

    The branch's own shape is described under *The docs site is Astro* below, because if it merges that is what the site is. The thing worth knowing before reading any of it: **all four faults the integration hit were silent**, and none was visible in a build log — a 4rem header reserve against a two-tier bar that put every `h1` under the sub-nav; a `position: fixed` header that reinstated at 375px exactly the bug `nav.css` dissolves the stack to prevent; an autogenerated sidebar that came out empty because `autogenerate` filters on `entry.filePath` and not `entry.id`; and a Pagefind index whose only UI lived in the header the site replaces. That class of problem is the standing cost of the integration, not a set of bugs that are now behind it.
...
is deliberate.** `docs/content.config.ts` renders `DESIGN.md`, `README.md`,
