...
-   [CCWEB2-317](https://codecave.atlassian.net/browse/CCWEB2-317) — **decided, and phase 1 landed** 2026-08-21 (`fbedb03`). `docs/` is built with Astro 7.2.4. The question it was blocked on — whether `docs/` stays committed as build output — is answered **no, and it could not have been yes**: `packages/brand` copies `docs/colors_and_type.css` byte-for-byte and asserts it, so making `docs/` an `outDir` would point the package's origin at generated output. Sources stay in `docs/`, output goes to gitignored `dist/`, Pages deploys `dist/`. That is *not* the shape ASTRO-MIGRATION.md §3 proposed, which predates the package. **Phase 2 landed** 2026-08-21 (`2603098`, `a128e87`): all thirteen `preview/` cards render, and `docs/preview/` holds only `preview.css`. Two hazards it turned up are now written down — a brace in body text opens an `.astro` expression, and an entity in a `title` gets escaped twice, which `DocPage` now throws on. **Three phases remain and the site runs half-migrated meanwhile** — a page is either an `.astro` under `docs/pages/` or a hand-written `.html` elsewhere in `docs/`, both served identically. Porting one means deleting the `.html` in the same commit; `docs/tools/astro-passthrough.mjs` fails the build if you forget, because Astro's own behaviour there is a `WARN` and exit 0. `artifacts/` is never rendered.
