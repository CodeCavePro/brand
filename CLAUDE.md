...
-   [CCWEB2-327](https://codecave.atlassian.net/browse/CCWEB2-327) — **fixed, awaiting a PR.** All 27 GSAP plugin imports pointed at `gsap/dist/…`, the UMD/CJS build; rolldown's `__toESM(mod, 1)` interop makes the default import the whole `module.exports`, so `ScrollTrigger.create` was `undefined` and section pinning was dead on eight pages. Branch `ccweb2-scrolltrigger-import`, commit `6ab39cc`.
-   [CCWEB2-328](https://codecave.atlassian.net/browse/CCWEB2-328) — **fixed, awaiting a PR.** 32 of the site's 33 pages shipped with no `<title>`, no description and no canonical, and the one canonical that existed said `http://localhost:4321/` — `SeoPage` built it from `Astro.url.origin`, which during a static build is the dev server. `SeoPage` now takes the full head (canonical, robots, Open Graph, Twitter card, article timestamps) off `Astro.site`, every page passes it, and `scripts/check-seo.mjs` fails the build if one does not. Setting `site` also un-broke `@astrojs/sitemap`, which had been silently skipping every build. Branch `ccweb2-328-seo-meta`, commits `58bd677e` and `de6cca75`. Tag casing came with it: rather than a dictionary mapping `ai`→AI and `devops`→DevOps, tags are lowercased as they leave Strapi (`src/helpers/tags.ts`), which needs no map and cannot drift — and changes no URL, because `createSlug` already lowercased before slugifying. Tag pages quote the tag (`Insights tagged “ai”`) so lowercase reads as deliberate in a search result. **The one thing still needing a human is the social card**, split out as [CCWEB2-329](https://codecave.atlassian.net/browse/CCWEB2-329) and assigned to Maria Shaban: the default `og:image` is the square PWA icon standing in for a real 1200×630 card, so every share letterboxes.
