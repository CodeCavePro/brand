...
-   [CCWEB2-334](https://codecave.atlassian.net/browse/CCWEB2-334) — two dead build config files, found while extracting the `@theme` block. `global.css`'s `@config` names `tailwind.config.js` where the repo has `tailwind.config.ts`, so it resolves to nothing — and **the obvious `.js`→`.ts` fix would unstyle every Vue component**, because that config's `content.files` glob has no `.vue` in it. `postcss.config.mjs` uses `module.exports` inside an ESM file, so autoprefixer is configured by nothing. Both are one-line deletions in the build path, which is why they are a PR of their own.
