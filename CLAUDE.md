...
### The storybook is output, and developing a component is two steps

**Edit under `src/components/` or `src/captured/`, and commit. That is the
whole process.** Nothing has to be regenerated by hand, because
`docs/storybook/compiled/*.js` and `docs/storybook/tw-bridge.css` are
**gitignored** — produced by `npm run build:storybook` on the same rule as
`packages/brand/dist/` and `dist/`: `src/` is authored, everything else is
generated.

To *see* the change, `npm run dev`. The kitchen-sink specimens import the
`.vue` sources through Vite and hot-reload, so the storybook output is not in
that loop at all.

**What the two outputs are for.** `compiled/*.js` are the SFCs compiled to plain
...
it needs a build first. `npm run check` already required one — it reads
`packages/brand/dist/` — so this is a second reason for a precondition that was
always there, and `static.yml` runs it after the rebuild rather than in the
dependency-free block.

**This replaced a tracked-artifact arrangement on 2026-08-27, and the deletion
is the point.** The thirteen files were committed and guarded by
`check-tw-bridge.mjs`, which compared a sha256 of both component roots against a
digest recorded in the `tw-bridge.css` header. That only ever made sense while
the generator needed a codecave.pro checkout and so could not run everywhere — a
node-only checker standing in for a build nobody could perform. The decoupling
ended that, and left the check doing something worse than nothing: **CI already
rebuilt the storybook and published the fresh output**, so a red check failed
the build over files the same run then discarded. It cost the 2.4.1 release and
a red `development` before it went, and its line-ending sensitivity had cost 36
consecutive Pages deploys in August. `tools/source-digest.mjs` went with it.

The rule it leaves behind is the general one, stated once: **a derived file is
not committed.** If something under `docs/` starts being generated, gitignore it
rather than writing a checker to notice when it rots.
