...
-   [CCWEB2-318](https://codecave.atlassian.net/browse/CCWEB2-318) — **epic.** Publish `@codecavepro/brand` to npm and have codecave.pro install it, inverting the direction of truth. **`@codecavepro/brand` 1.0.0 and 1.1.0 are both on public npm (2026-08-21) — phase 5 is done. Phase 6 is written and pushed: branch `ccweb2-318-brand-tokens` on `CodeCavePro/codecave.pro`, commit `482e96f1`. Open: opening that PR — there is no `gh` here, so it is a human click — and it is to be opened, not merged. Then phase 4.** The epic lists phase 4 before phase 6; **that order was reversed on 2026-08-21 and the reason is below.** Re-measure the captures with `npm run check:captures` before building components from them; never publish a component built from a capture that is behind the site.
