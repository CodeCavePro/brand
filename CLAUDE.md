...
Genuine bugs in what codecave.pro ships — most found while resyncing the
captures, the rest while working on the site itself.
**These are the site's to fix, not the package's** — do not "correct" them in
`docs/`. Items marked *fixed* are on a site branch and named here only so the
next person does not re-find them; the fix itself lives in the site repo:

-   [CCWEB2-319](https://codecave.atlassian.net/browse/CCWEB2-319) — **fixed, on branch `ccweb2-small-site-bugs` (`1d154af0`), not yet on `development`.** The site's `h-11` sat under `min-h-12`, and `min-height` beats `height`, so it was dead — deleting it moved nothing. The package half was already settled: `--control-height` is `3rem`/48px, applied by `.btn` as `min-height`. **One divergence found and deliberately not changed:** `GlowButton.vue:69` carries `h-11` with *no* `min-h-12`, so it really does render 44px. Fixing that is a visible 4px change outside the ticket's scope; it is noted on CCWEB2-319 for a human to decide.
-   [CCWEB2-320](https://codecave.atlassian.net/browse/CCWEB2-320) — `TextField.vue`'s error message renders at 2.91:1. Accessibility.
-   [CCWEB2-321](https://codecave.atlassian.net/browse/CCWEB2-321) — **fixed** in the same commit. The footer label read "Truspilot"; the `href` was always right.
-   [CCWEB2-322](https://codecave.atlassian.net/browse/CCWEB2-322) — **fixed** in the same commit. `LazyImage.vue` declared `width`/`height` and never bound them, so every lazy image on every page was an unreserved box — a layout shift when the observer swapped `data-src` in, not merely an unused prop.
-   [CCWEB2-324](https://codecave.atlassian.net/browse/CCWEB2-324) — **needs no code change; it was already fixed.** The CCWEB2-325 rewrite removed the dead `astro:content` import along with the rest of the old `ContactUsForm`. Verified across the tree, not just that file: `grep -rn "astro:content" src/ --include=*.vue` matches nothing.
-   [CCWEB2-326](https://codecave.atlassian.net/browse/CCWEB2-326) — **fixed, on branch `ccweb2-small-site-bugs` (`ad7b2419`).** `astro check` type-checks `.astro` and **not** `.vue`, so it had been reporting zero errors on files it never opened. `vue-tsc --noEmit` is now part of `build`; it found five errors in four files on its first run, all fixed. The guard was watched failing (injected error → exit 2) before being trusted — the same bar as `check:seo` and `check:ports`.
-   [CCWEB2-327](https://codecave.atlassian.net/browse/CCWEB2-327) — **fixed and on `development`** (`6ab39cca`). All 27 GSAP plugin imports pointed at `gsap/dist/…`, the UMD/CJS build; rolldown's `__toESM(mod, 1)` interop makes the default import the whole `module.exports`, so `ScrollTrigger.create` was `undefined` and section pinning was dead on eight pages. Branch `ccweb2-scrolltrigger-import`; the commit is an ancestor of `origin/development`, so it is deployed to the development Cloudflare environment.
-   [CCWEB2-328](https://codecave.atlassian.net/browse/CCWEB2-328) — **fixed and on `development`** (`58bd677e`, `de6cca75`). 32 of the site's 33 pages shipped with no `<title>`, no description and no canonical, and the one canonical that existed said `http://localhost:4321/` — `SeoPage` built it from `Astro.url.origin`, which during a static build is the dev server. `SeoPage` now takes the full head (canonical, robots, Open Graph, Twitter card, article timestamps) off `Astro.site`, every page passes it, and `scripts/check-seo.mjs` fails the build if one does not. Setting `site` also un-broke `@astrojs/sitemap`, which had been silently skipping every build. Branch `ccweb2-328-seo-meta`, whose commits are also on `origin/development` — a PR from it would show an empty diff. Tag casing came with it: rather than a dictionary mapping `ai`→AI and `devops`→DevOps, tags are lowercased as they leave Strapi (`src/helpers/tags.ts`), which needs no map and cannot drift — and changes no URL, because `createSlug` already lowercased before slugifying. Tag pages quote the tag (`Insights tagged “ai”`) so lowercase reads as deliberate in a search result. **The one thing still needing a human is the social card**, split out as [CCWEB2-329](https://codecave.atlassian.net/browse/CCWEB2-329) and assigned to Maria Shaban: the default `og:image` is the square PWA icon standing in for a real 1200×630 card, so every share letterboxes.
-   [CCWEB2-330](https://codecave.atlassian.net/browse/CCWEB2-330) — **filed, not implemented.** 32 distinct internal link targets — the whole nav, the whole footer, every service, article, project, tag and author link — are written without a trailing slash while `build.format` is `'directory'`, so each one is a redirect hop. Nothing complained because Astro's `trailingSlash` defaults to `'ignore'`. Scoped it as "the tag chips" at first; measuring showed it is most links on all 34 pages.
