...
-   [CCWEB2-330](https://codecave.atlassian.net/browse/CCWEB2-330) — **fixed on a branch, waiting on a merge into the site.** Every internal link — 716 of them, 32 targets, all 34 pages — omitted the trailing slash while `build.format` is `'directory'`, so each was a 308 hop (verified live). `trailingSlash: 'always'` and slashes in `paths.ts` fix it on `ccweb2-330-trailing-slashes`. **The lesson is the second commit, not the first:** `paths.ts` values also feed JSON-LD `@id`, `og:url` and share links, so the `href`-only guard passed a build that had shipped `/insights//` to nine pages of structured data. A check that reads one syntax is not a check on the thing it names.
...
-   [CCWEB2-334](https://codecave.atlassian.net/browse/CCWEB2-334) — **fixed on a branch, waiting on a merge into the site.** Three dead build configs, not the two the ticket names. `postcss.config.mjs` uses `module.exports` inside an ESM file, and nothing loads it anyway — `astro.config.mjs` passes an inline `vite.css.postcss`, which makes Vite skip the config-file search. `global.css` opened with `@config "../../tailwind.config.js"` where the repo has `tailwind.config.ts`, and **the obvious `.js`→`.ts` fix would unstyle every Vue component**, because that config's `content.files` glob has no `.vue` in it. And `tailwind.config.ts` is loaded by neither route: the `config:` option `astro.config.mjs` hands `tailwindcss()` does not exist — `@tailwindcss/vite` 4.3.0 takes `{optimize?}` and nothing else. All three are deleted on `ccweb2-334-dead-build-configs`, with all 135 built files byte-identical before and after.
-   [CCWEB2-339](https://codecave.atlassian.net/browse/CCWEB2-339) — **fixed on a branch, waiting on a merge into the site.** The deploy workflow ran `npm install` in a repo that commits `pnpm-lock.yaml` and no `package-lock.json`, so CI installed with **no lockfile at all** and re-resolved every range on each deploy. That matters here specifically: the site asks for `@codecavepro/brand: ^1.1.0`, the lockfile pins **1.1.0**, and npm `latest` is **1.4.0** — so production has been on 1.4.0 while every checkout is on 1.1.0, with no commit recording it. Harmless so far by luck: the site's only import is `tokens.css` and that file is byte-identical across those versions. Nothing holds that true for the next token change. Do **not** read the site lockfile as a statement about production until this merges. `ccweb2-dependabot-transitives` switches CI to `pnpm install --frozen-lockfile`; builds under 1.1.0 and 1.4.0 were compared file-by-file first, all 135 identical.
-   [CCWEB2-338](https://codecave.atlassian.net/browse/CCWEB2-338) — **fixed on the same branch.** The five Dependabot alerts, all transitive build tooling. Four moved by plain semver; the fifth could not — `yaml-language-server` pins `yaml` at exactly `2.7.1` — and needed an override. It goes in `pnpm-workspace.yaml`: **pnpm 11 no longer reads the `pnpm` field in `package.json`**, it warns and ignores the block.
