 ...
 *
 * Both outputs are GITIGNORED. They are produced from src/, like everything
 * else that is not authored, so there is nothing committed for them to drift
 * from and no digest to verify. That replaced a tracked-artifact arrangement
 * whose whole justification -- that this generator could not run without a
 * codecave.pro checkout, so a node-only checker had to stand in for it -- died
 * with the decoupling on 2026-08-26.
 *
 ...
 *
 * The practical half: this script and the Pages workflow both needed a second
 * private repository to be reachable. It was not -- CI's token has been
 * returning 403 -- so the rebuild step had never once run. That is also why
 * the storybook output was tracked and digest-checked at all; once this script
 * ran anywhere, both the tracking and the check went (2026-08-27).
 */
 ...
 * The site's Tailwind v4 theme (src/captured/styles/global.css @theme)
 ...
 *
 * Gitignored: produced from src/components/ and src/captured/ on every build.
 */
` + built + canvasScope + '\n';
