...
| 3.6  | `common/technology-card.vue`      | `index?: number` is optional but `rotate[index]`/`translate[index]` are unguarded — the mobile carousel really does omit it, interpolating the literal string `undefined` into classes | default the prop, guard the lookup                         |
...
- **The gray ramp is non-monotonic.** `gray-1000 #050505` is _darker_ than
...
- **The brand ramp hides neutrals and one inversion.** `brand-950 #0A0A0B` (the
  ...
  as violet steps but aren't. `brand-500 #5F20FE` is slightly _lighter_ than
...
- **Raw ramps in bare `:root`, semantics in `@theme`.** This works only because
  ...
  repaints Tailwind's _default_ gray palette for any future `text-gray-*`
